...
function aa = acc2Twin(kt,kc,kr,ks,kd,csvFile)
	...
	an_detected = abs(an_int) > 0.8*max(an_int);
	...
	% b_rectanle = abs((r_mat_order(2,str(1).C_ind(1))-500) - (r_mat_order(5,str(end).C_ind(end-1))+500));
	% h_rectanle = abs((r_mat_order(3,str(1).C_ind(1))-500) - (r_mat_order(6,str(end).C_ind(end-1))+500));
	...
	box on
	axis equal
	% xlim([r_mat_order(2,str(1).C_ind(1))-500 r_mat_order(5,str(end).C_ind(end-1))+500])
	% ylim([r_mat_order(3,str(1).C_ind(1))-500 r_mat_order(6,str(end).C_ind(end-1))+500])
	plot(r_s(1,:),r_s(2,:),'b','LineWidth',3)
	for i = 1:length(ind)-1
	...
	plot(r_s(1,:),r_s(2,:),'b','LineWidth',3)
	plot(r_mat_order(2,1),r_mat_order(3,1),'-og','LineWidth',2)
	for i = 1:50:length(time_tot)
	    
	    plot(r_s(1,i),r_s(2,i),'*k','LineWidth',2)
	    text(r_s(1,i)+5, r_s(2,i)+0, strcat('$s = $',num2str(s_tot(i)),'[m]'),'interpreter','latex','FontSize',10);
	...
	saveas(three, fullfile(base_folder, 'three_new.png'))
	save_name =fullfile(base_folder, 'three_new.svg');
	set(gcf,'PaperPositionMode','auto')
	%print(save_name,'-dsvg')
	print(gcf, save_name, '-dsvg')
	...
	box on
	axis equal
	xlim([min(rx)-100 max(rx)+100])
	ylim([min(ry)-100 max(ry)+100])
	xlabel('x [m]','interpreter','latex','FontSize',14)
	ylabel('y [m]','interpreter','latex','FontSize',14)
	ax = gca;
	ax.TickLabelInterpreter = 'latex';
	...
	saveas(three, fullfile(base_folder, 'four_new.png'))
	save_name =fullfile(base_folder, 'four_new.svg');
	set(gcf,'PaperPositionMode','auto')
	%print(save_name,'-dsvg')
	print(gcf, save_name, '-dsvg')
	...
	saveas(three, fullfile(base_folder, 'five_new.png'))
	save_name =fullfile(base_folder, 'five_new.svg');
	set(gcf,'PaperPositionMode','auto')
	print(gcf, save_name, '-dsvg')
	...
	saveas(three, fullfile(base_folder, 'six_new.png'))
	save_name =fullfile(base_folder, 'six_new.svg');
	set(gcf,'PaperPositionMode','auto')
	%print(save_name,'-dsvg')
	print(gcf, save_name, '-dsvg')
